...
//成男or成女or少女
var idx = 0;
$(".xw-cut-list a").on("click", function () {
    idx = $(this).index();
    $("#pop4 .pop-con .pop-cn-img").hide().eq(idx).show();
    TGDialogS('pop4');
});

$("#pop4 .prev-btn").on("click", function () {
    idx--;
    if (idx < 0) idx = 2;
    $("#pop4 .pop-con .pop-cn-img").hide().eq(idx).show();
});
$("#pop4 .next-btn").on("click", function () {
    idx++;
    if (idx > 2) idx = 0;
    $("#pop4 .pop-con .pop-cn-img").hide().eq(idx).show();
});
