...
var typeName = 0; //分享类型 [main index page]
function isWeiXin() {
...
function oneShare(typename) {
    need("biz.login", function (LoginManager) {
        LoginManager.checkLogin(function (userinfo) {
            ...
            amsCfg_689931.sData.typeName = ((typename == 0) ? 'main' : ((typename == 1) ? 'index' : 'page'));
...
function shareFun(shareType) {
    ...
    if (shareType == 1) {
        ...
        typeName = 2;
        if (player.inviteCodeToken == '' || player.inviteCode == '' || player.inviteCodeToken == undefined || player.inviteCode == undefined) {
    ...
    need("biz.login", function (LoginManager) {
        LoginManager.checkLogin(function (userinfo) {
            ...
            if ((isWeiXin() && acctype != 'wx') || (isQQ() && acctype == 'wx')) {
                ...
                if (false) {
                    $(".banner-wrap").hide();
                    //localStorage.removeItem("f_1001");
                }
...
amsCfg_689267 = {
    ...
    "fFlowSubmitEnd": function (res) {
        if (res.iRet == 0) {
            ...
            cjStatus = res.sOutValue4.split('|'); //刮奖
            initPage();
        } else {
            alert('系统繁忙，请刷新页面');
        }
...
amsCfg_690214 = {
    ...
    "fFlowSubmitEnd": function (res) {
        if (res.iRet == 0) {
            if (res.jData.inviteCode != '' && res.jData.inviteCode !== undefined) {
            ...
            if (isQQ()) {
                ...
                orderGameCenter({
                    ...
                    callback: function (json) {
                        ...
                        if (ret == '0') {
                            amsCfg_690214.sData = sData;
                            amsCfg_690214.sData.extend1 = 11;
                            amsSubmit(321796, 690214);
                        } else if (ret == '1995001') {
                            amsCfg_690214.sData = sData;
                            amsCfg_690214.sData.extend1 = 22;
                            amsSubmit(321796, 690214);
                        }
                    }
                });
