...
var player = {isBind: false,inviteCode:'',shareType:1}; //玩家基础信息
    ...
    "fFlowSubmitEnd": function(res){
        ...
        if (res.iRet == 0) {
            ...
            if (res.jData.rsvtStatus != '1'){
                ...
                $("#popTips .pop-msg-text").text('您已经预约过了');
                TGDialogS('popTips');
            }
        }
        return;
...
amsCfg_691083 = {
    ...
    "fFlowSubmitEnd": function(res){
        ...
        if (res.iRet == 0 && res.jData.ret == 0) {
            ...
            var alertUrls = ['qq.com','qlogo.cn','qpic.cn','gtimg.cn'];
            if(String(res.jData.extend2).indexOf(alertUrls[0]) >= 0 || String(res.jData.extend2).indexOf(alertUrls[1]) >= 0 || String(res.jData.extend2).indexOf(alertUrls[2]) >= 0 || String(res.jData.extend2).indexOf(alertUrls[3]) >= 0){
