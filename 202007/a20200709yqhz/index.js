var ctrlJs = {
    ...
    methods: {
        ...
        lookYqCard: function(obj){
            ...
            for(var i=0;i<3;i++){
                src[i] = '//game.gtimg.cn/images/gp/cp/a20200709yqhz/poster_kh'+ (obj.idx+1) +'_'+ ctrlJs.data.mxList[arr[i]] +'.jpg';
                createImg(i, src[i]);
            }
                ...
                ctrlJs.methods.getBase64(val).then(base64 => {
                    ...
                    if(wcIdx >= 3){
                        ...
                        setTimeout(function(){
                            ...
                            ctrlJs.data.lookSwiper = ctrlJs.methods.initSwiper('.J-ck-khk', obj.status);
                        }, 500);
                    }
                }, err => {
                    console.log(err);
        ...
        fxYqCard: function(obj){
            ...
            arr.sort(function(){return Math.random() > 0.5 ? -1 : 1;});//随机卡片排序
            for(var i=0;i<3;i++){
                src[i] = '//game.gtimg.cn/images/gp/cp/a20200709yqhz/poster_kh'+ (obj.idx) +'_'+ ctrlJs.data.mxList[arr[i]] +'.jpg';
                createImg(i, src[i]);
            }
            function createImg(idx, val){
                ctrlJs.methods.load();
                ctrlJs.methods.getBase64(val).then(base64 => {
                    ...
                    if(wcIdx >= 3){
                        ...
                        setTimeout(function(){
                            ctrlJs.methods.close();
                            ctrlJs.data.fxSwiper = ctrlJs.methods.initSwiper('.J-fx-khk', 1);
                        }, 500);
                    }
                }, err => {
                    console.log(err);
        ...
        fxMxcard:function(obj){
            var img = '//game.gtimg.cn/images/gp/cp/a20200709yqhz/poster_mx_'+ ctrlJs.data.mxList[obj.id]+'.jpg';
            ctrlJs.methods.load();
        ...
        playVideo: function(idx){
            var src = mediaURLData[ ctrlJs.data.videoList[idx] ];
                $video = ctrlJs.data.$video;
            console.log(idx);
            $video.attr('src', src);
            $video[0].play();
            $video[0].addEventListener("ended",function(){
                if(idx<ctrlJs.data.videoList.length-1){
                    ctrlJs.methods.playVideo(idx+1);
                }else {
                    $video[0].pause();
                    closeDialog();
                }
    ...
    init: function (callback) {
        ...
        $(".part-1").css('background-image', 'url(//game.gtimg.cn/images/gp/cp/a20200709yqhz/p1_bg_'+ bg +'.jpg)');
...
$(function () {
    ...
    var obj = {
        idx: 1, //元气卡编号 总共8张
        name: '元', //元气卡名称
        src: '//game.gtimg.cn/images/gp/cp/a20200709yqhz/card_kh_1.png',//默认元气卡图片地址
    };
    // setTimeout(function(){
    ...
    //     var title = "发现口号卡 - 元",
    //         imgSrc = "//game.gtimg.cn/images/gp/cp/a20200709yqhz/card_kh_1.png";
    ...
    $(".part-2 .card-kh-list li").on("click", function(){
        ...
        // if(obj.cardNum == 0){
        //     $("#gx_btn_tip").addClass("gray").removeAttr("href");//置灰共享按钮
        // }
    ...
    $(".part-2 .card-mx-list li").on("click", function(){
        var $this = $(this).find('img'),
            ...
            img = '//game.gtimg.cn/images/gp/cp/a20200709yqhz/poster_mx_'+ ctrlJs.data.mxList[id]+'.jpg';
        ctrlJs.methods.load();
        ctrlJs.methods.getBase64(img).then(base64 => {
            ctrlJs.methods.close();
