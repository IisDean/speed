...
Component({
  ...
  methods: {
    ...
    drawCircleBg: function (id, x, w) {
      ...
      ctx.beginPath();//开始一个新的路径
      //设置一个原点(x,y)，半径为r的圆的路径到当前路径 此处x=y=r
      ctx.arc(x, x, x - w - 6, 0, 2 * Math.PI, false);
      ctx.setLineWidth(1);
      ctx.strokeStyle = "#666"
      ctx.stroke();//对当前路径进行描边
      ctx.draw();
    ...
    drawCircle: function (id, x, w, step) {
      ...
      gradient.addColorStop("0", "#3897fe");
      ...
      // step 从0到2为一周
      context.arc(x, x, x - w/2, -Math.PI / 2, step * Math.PI - Math.PI / 2, false);
