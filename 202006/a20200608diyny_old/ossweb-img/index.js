var ctrlJs = {
    ...
    methods: {
        ...
        createResult: function(){
            ...
            window.sessionStorage.setItem('resultData', resultData);
            ctrlJs.methods.nextPageRead(5);
            setTimeout(function(){
                $(".part-init").removeClass("part-show");
    ...
    init: function(){
        ...
        $(".part-2 .btn-next").on("click", function(){
            if(that.data.jd <= 0)return alert('先选择一种基底吧~');
        ...
        $(".part-3 .btn-next").on("click", function(){
            if(that.data.gz.length < 3)return alert('需要选择3种果汁哦~');
        ...
        $(".part-4 .btn-next").on("click", function(){
            if(that.data.zl.length < 3)return alert('需要选择3种佐料哦~');
            // 生成结果
